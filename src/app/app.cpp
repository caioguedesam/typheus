...
m4f GetWorldToLightSpaceMatrix(DirectionalLight* light)
{
    ...
    m4f lightProj = OrthographicProjectionMatrix(-10, 10, -10, 10, light->nearPlane, light->farPlane);
...
void App_Render()
{
    ...
    if(inEditorMode)
    {
        ...
        {
            Camera& cam = Renderer_GetCamera();
            ImGui::Text("Camera pos: (%.2f, %.2f, %.2f)", cam.position.x, cam.position.y, cam.position.z);
            ImGui::Text("Camera dir: (%.2f, %.2f, %.2f)", cam.front.x, cam.front.y, cam.front.z);
            ...
            if(ImGui::TreeNode("Directional Light"))
            {
                ...
                ImGui::DragFloat("Near Plane", &directionalLight.nearPlane, 0.005f, -1000.f, 1000.f);
                ImGui::DragFloat("Far Plane", &directionalLight.farPlane, 0.005f, 1000.f, 1000.f);
                ImGui::TreePop();
            }
